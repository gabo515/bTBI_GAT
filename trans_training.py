...
# hyperparams
################################################################################
pdfp = r'C:\Users\Gabog\Downloads\bTBI Project'
data_pkl = 'bTBI_pickle_2023_04_17.pkl'
replicate='0' #sys.argv[1] # for pkl file saved
...
#node_features = torch.from_numpy(datapkl.X).float() # already dense, 
node_features = torch.from_numpy(datapkl.obsm['features'].todense()).float()
# _,node_features = scipysparse2torchsparse(features)
labels = torch.LongTensor(datapkl.obs['bTBI'])
edge_index, _  = scipysparse2torchsparse(datapkl.obsm['adj'])
...
for epoch in range(nEpochs):
    ...
    torch.save(model.state_dict(), f'{epoch}-'+replicate+'.pkl')
